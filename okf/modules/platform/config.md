...
description: Single source of truth for runtime configuration, loaded once from the environment, validated at startup, and passed down so no other package reads env vars.
resource: go/internal/config
...
- **GitHub auth mode** (`resolveGitHubApp`): with no `GITHUB_APP_*` vars set, the
  ...
  installation-token choice is realized in [auth](/modules/platform/auth.md).
- See [architecture design](/standards/architecture-design.md) §12 and `.env.example`
