...
title: Notifications
description: Provider-agnostic notifier that posts Messages to Slack or Microsoft Teams behind one interface, chosen by a config flag.
resource: go/internal/notify
...
`Notifier` interface, so the choice is a config flag (`NOTIFY_PROVIDER`, read by
[config](/modules/platform/config.md)).
...
- `teams.go` — Teams **Workflows / Adaptive Card** format (the O365 connector
  MessageCard path is deprecated; this service targets the new one).
- `New(provider, slackURL, teamsURL)` picks the implementation.

Deterministic tooling — no agent imports. Tested with a local HTTP-stub server
capturing the posted body; no real Slack/Teams calls.
