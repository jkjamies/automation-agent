...
The lens table also carries what each lens cost. `setup.DriveReport` attributes every non-partial event of a drive to its author agent — wall time from the start of the drive to the agent's last event (for the parallel fan-out this includes any wait for a model concurrency slot: it is the time the lens took as experienced, not model time), the token usage the model reported (`UsageMetadata`, which the Ollama adapter maps from the server's `prompt_eval_count` / `eval_count` and the Gemini adapter reports natively), and the model version the response named, falling back to the configured model's name. A lens that produced no output at all is marked **no output** rather than reading as clean; token cells show `–` when the model reported no usage, which is not the same as zero.
