...
- **Staleness at execution** (`Kickoff` → `superseded`): the engine fetches the PR's current head SHA and skips if it no longer matches the event's SHA (a newer push won). Best-effort — a lookup error proceeds rather than suppressing a real review.
  ...
  This is checked at **every stage boundary**, not once: at intake, after standards discovery, after the lens fan-out (before glue), and again before publish. Debounce and the intake check together cover the *queue* window, but a review takes minutes, so a push can easily land while one is already running — and then the two are concurrent, on different SHAs.
