...
type: Standard
title: Deployment
description: The canonical deployment and operations reference — HTTP surface, execution transport, GitHub App auth, step-by-step GCP setup, and the private-ingress architecture.
...
> [Automation Agent — Architecture & Build Plan](/standards/architecture-design.md) §8, §13 (design).
...
[Local development § Environment variables](/standards/local-development.md#environment-variables-full-reference).
The vars that matter specifically for a **cloud** deploy:
...
[planned hardening](#planned-hardening); the setup steps above are manual.)
...
- **Collector fan-out (recommended for multi-vendor flexibility).** The app always exports
  ...
  span-export flush constraint (see [Observability](/standards/observability.md)) is independent
  of which shape you pick: the app force-flushes in-request either way, so scale-to-zero never
...
GCP-native cron are retained. Selecting this posture is config + infra only; rollout detail
...
concrete product selection is recorded in `DEPLOYMENT.md`.
