---
type: Standard
title: CI Integration — sending lint problems to the agent
description: How a CI pipeline on any tech stack kicks off the lint and coverage fixers and wires the verify checks that resume them.
...
and an LLM triage step normalizes it (see
[Automation Agent — Architecture & Build Plan](/standards/architecture-design.md) §8).
You only need to send a small, trusted envelope.
...
webhooks** to wire (see [Deployment](/standards/deployment.md)). The agent then:
...
[Automation Agent — Architecture & Build Plan](/standards/architecture-design.md) §8 and
`DEPLOYMENT.md`.)
...
requests** (the reviewer's native-event kickoff — see
[Webhooks & CI check names](/standards/webhooks.md))) — not per repo. The handler routes
the single URL by `X-GitHub-Event`: `check_run` → fixer resume, `pull_request` →
`KindReview`. See the
[Deployment § GitHub App setup](/standards/deployment.md#github-app-setup-production-auth).
