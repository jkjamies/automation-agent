---
type: Architecture
title: Automation Agent — Architecture & Build Plan
description: The authoritative language-neutral design of the automation agent — goals, topology, durable suspend/resume sessions, configuration, and deployment.
tags: [architecture, design, durable-sessions, agents]
...
10. [ARCH tests, the knowledge bundle, specs, Makefile](#10-arch-tests-the-knowledge-bundle-specs-makefile)
...
(it opens no PR/branch). See §8 and [Webhooks & CI check names](/standards/webhooks.md).
...
│   │   └── docs_test.go           # okf bundle conformance (frontmatter, indexes, links)
│   │
│   └── internal/
...
## 10. ARCH tests, the knowledge bundle, specs, Makefile
...
- **ARCH/** — `archtest`-style assertions:
  ...
  - A second test (`docs_test.go`) asserts the `okf/` knowledge bundle conforms: every
    concept carries a frontmatter `type`, every bundle directory has an `index.md`,
    bundle-absolute links resolve, and the repo-root `AGENTS.md` points at `okf/index.md`.
- **The knowledge bundle** — the system's knowledge lives in the repo-root `okf/` bundle
  (markdown concepts with YAML frontmatter, cross-linked). The single repo-root
...
- **Docs + diagrams move with the code (a change is not done until they do).** `docs_test`
  ...
  ingress route, or tooling package changes, sweep every place that describes or draws it
...
and the firestore emulator for local tests — lives in `DEPLOYMENT.md`; the design-level summary:
...
- **Observability (`OTEL_TRACES_EXPORTER`).** The agent framework already emits a native
  ...
  a Collector). Design in [Observability (distributed tracing)](/standards/observability.md).
...
1. **Skeleton & standards** — repo tree, go.mod, Makefile, the `okf/` knowledge bundle +
   spec templates, ARCH tests, the root `AGENTS.md` guardrail sheet, config, ingest envelope.
