...
- Webhook bodies are **size-capped per route class** (reject, not truncate — a truncated
  body feeds malformed JSON downstream, and on the HMAC routes fails signature verification
  too when a secret is set); oversized → 413, not 500, so a caller cannot retry a body that
  can never fit. See [webhook](/modules/platform/webhook.md).
