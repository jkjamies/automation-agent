...
description: The import-boundary and durable-session state rules the ARCH test suite enforces identically across every port.
...
**language-neutral**: they hold identically across every port (`go/`, `python/`, `kotlin/`,
`javascript/`). The **durable-session state** model below is the design for the fix-loop's
suspend/resume state. See [Language parity](/standards/language-parity.md) for the
cross-language 1:1 contract; any deliberate divergence between ports is recorded in the PR
that introduces it.
